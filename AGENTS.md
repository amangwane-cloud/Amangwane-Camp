...
Booking is enquiry-based: every "Book Now" CTA links to the contact form at
`index.html#contact` and carries a `data-book` attribute, which is the selector the
conversion tracking uses. There is no third-party booking widget.
